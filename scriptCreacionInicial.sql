...
/* ESTO ASI FUNCA PERO ME TRIPLICA LA CANT
...
JOIN VAMONIUEL.CRUCERO CRU ON (M.CRUCERO_MODELO = cru.CRUCERO_MODELO)
...
(SELECT TOP 1 CRU.ID FROM VAMONIUEL.CRUCERO CRU WHERE M.CRUCERO_MODELO = cru.CRUCERO_MODELO), 
(SELECT TOP 1 R.ID FROM VAMONIUEL.RECORRIDO R  WHERE M.PUERTO_DESDE = R.PUERTO_DESDE AND M.PUERTO_HASTA = R.PUERTO_HASTA)
FROM gd_esquema.Maestra M 
JOIN VAMONIUEL.CRUCERO CRU ON (M.CRUCERO_MODELO = cru.CRUCERO_MODELO)
JOIN VAMONIUEL.PASAJE P ON 
(M.PASAJE_CODIGO = P.PASAJE_CODIGO  AND M.PASAJE_FECHA_COMPRA = P.PASAJE_FECHA_COMPRA AND M.PASAJE_PRECIO = P.PASAJE_PRECIO) 

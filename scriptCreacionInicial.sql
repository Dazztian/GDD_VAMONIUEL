...
SELECT M.PUERTO_DESDE, M.PUERTO_HASTA, M.FECHA_SALIDA, M.FECHA_LLEGADA,P.ID, CRU.ID,R.ID
FROM gd_esquema.Maestra M 
JOIN VAMONIUEL.RECORRIDO R ON ( M.PUERTO_DESDE = R.PUERTO_DESDE AND M.PUERTO_HASTA = R.PUERTO_HASTA)
LEFT JOIN VAMONIUEL.CRUCERO CRU ON ( M.CRU_FABRICANTE= CRU.CRU_FABRICANTE AND M.CRUCERO_IDENTIFICADOR = CRU.CRUCERO_IDENTIFICADOR AND M.CRUCERO_MODELO = CRU.CRUCERO_MODELO )
LEFT JOIN VAMONIUEL.PASAJE P ON (M.PASAJE_CODIGO = P.PASAJE_CODIGO  AND M.PASAJE_FECHA_COMPRA = P.PASAJE_FECHA_COMPRA AND M.PASAJE_PRECIO = P.PASAJE_PRECIO) 

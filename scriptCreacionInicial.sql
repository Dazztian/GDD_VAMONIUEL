...
--Aca hay un drama con el id crucero xq no me lo puede ligar con ningun otro dato x eso llega como NULL!!!
DELETE FROM VAMONIUEL.VIAJE

INSERT INTO [VAMONIUEL].[VIAJE]([Origen],[Destino],[FechaInicio],[FechaFin],[CRUCERO_IDENTIFICADOR],[ID_Pasaje],[ID_Crucero],[ID_Recorrido])
SELECT M.PUERTO_DESDE, M.PUERTO_HASTA, M.FECHA_SALIDA, M.FECHA_LLEGADA,M.[CRUCERO_IDENTIFICADOR],P.ID,CRU.ID, R.ID
FROM gd_esquema.Maestra M 
JOIN VAMONIUEL.RECORRIDO R ON ( M.PUERTO_DESDE = R.PUERTO_DESDE AND M.PUERTO_HASTA = R.PUERTO_HASTA)
LEFT JOIN VAMONIUEL.CRUCERO CRU ON (M.CRUCERO_MODELO = cru.CRUCERO_MODELO)
LEFT JOIN VAMONIUEL.PASAJE P ON --Si le dejo el left me va a generar un viaje incluso para las reservas, lo cual para la migracion es correcto
